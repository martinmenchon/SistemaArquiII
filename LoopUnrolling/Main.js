...
"-  El tipo de un areglo puede ser double o int.\n"+
"-  Como máximo se puede definir un arreglo de cada tipo.\n\n"+
...
function ejecutar(){
	...
	var textoAss="";
	returnToken(document.getElementById("textAreaBig").value,document.getElementsByName("bucles")[0].value);
...
function loopUnrolling(cantUnrolling, listaAssembler) {
...
//	var indice = 1;
//	while (listaAssembler[indice - 1] != "LAZO:") {
//		indice++;
//	}
	var indice=0;
	var cortar=false;
...
//	for (k = 0; k < listaLoopUnrolling.length; k++) {// SACAAAAR!!!
//		console.log(listaLoopUnrolling[k]);
//
//	}
...
function returnToken(lines,cantUnrolling) {

//	console.log(lines);// SACAR
	// console.log(declareArray(lines,500));//sacar ya
	...
	try {
		...
		if (Gramatica.parse(getFor(lines))) {
			...
			loopUnrolling(cantUnrolling, listaAssembler);
			
			listaAssembler.push("\n \n Cantidad de ciclos: "+(calcularCantidadCiclosTarda(listaAssembler) * numCiclos));

			
//			console.log(" ");
//			console.log("CODIGO ASSEMBLER; ");
//			for (i = 0; i < listaAssembler.length; i++) {// SACAAAAR!!!
//				console.log(listaAssembler[i]);
//
//			}
//
//			console.log("Cantidad de ciclos que tarda: "
//					+ calcularCantidadCiclosTarda(listaAssembler) * numCiclos);
